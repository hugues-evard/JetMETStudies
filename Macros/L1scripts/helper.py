...
def makehistosforturnons_inprobeetaranges(df, histos, etavarname, phivarname, ptvarname, responsevarname, etabins, l1varname, l1thresholds, prefix, binning, l1thresholdforeffvsrunnb, offlinethresholdforeffvsrunnb, nbbins, runmin, runmax):
    ...
    for i in range(len(etabins)-1):
        ...
        histos[prefix+str_bineta+'_ResponseVsRunNb'] = df_etarange.Histo2D(
                ROOT.RDF.TH2DModel('h_ResponseVsRunNb_{}_{}'.format(prefix, str_bineta), 
                    '', nbbins, runmin, runmax, 100, 0 , 2), 
                'runnb', 'response')
        ...
        histos[prefix+"_plateaueffvsrunnb_numerator_"+str_bineta] = df_etarange.Histo1D(
                ROOT.RDF.TH1DModel(
                    'h_PlateauEffVsRunNb_Numerator_{}_{}'.format(prefix, str_bineta), 
                    '', nbbins, runmin, runmax),
                'runnb_inplateaupassL1')
        histos[prefix+"_plateaueffvsrunnb_denominator_"+str_bineta] = df_etarange.Histo1D(
                ROOT.RDF.TH1DModel(
                    'h_PlateauEffVsRunNb_Denominator_{}_{}'.format(prefix, str_bineta),
                    '', nbbins, runmin, runmax),
                'runnb_inplateau')
...
def ZEE_Plots(df, nbbins, runmin, runmax):
    histos = {}
    ...
    for i in range(3): 
        ...
        df_eg[i] = df_eg[i].Define('probe_L1Pt_Bx0',
                'GetVal(probe_idxL1jet_Bx0, _L1eg_pt)')

        df_eg[i] = df_eg[i].Define('probe_L1Pt_Bxmin1',
                'GetVal(probe_idxL1jet_Bxmin1, _L1eg_pt)')
        ...
        if i ==0:
            ...
            df_eg[i] = df_eg[i].Define('probeL1EG15to26Bxmin1_Eta',
                    'probe_Eta[probe_Pt>12&&probe_Pt<23&&probe_L1Pt_Bxmin1>15&&probe_L1Pt_Bxmin1<=26&&probe_L1Bx==-1]')
            df_eg[i] = df_eg[i].Define('probeL1EG15to26Bxmin1_Phi',
                    'probe_Phi[probe_Pt>12&&probe_Pt<23&&probe_L1Pt_Bxmin1>15&&probe_L1Pt_Bxmin1<=26&&probe_L1Bx==-1]')
            df_eg[i] = df_eg[i].Define('probeL1EG15to26Bx0_Eta',
                    'probe_Eta[probe_Pt>12&&probe_Pt<23&&probe_L1Pt_Bx0>15&&probe_L1Pt_Bx0<=26&&probe_L1Bx==0]')
            df_eg[i] = df_eg[i].Define('probeL1EG15to26Bx0_Phi',
                    'probe_Phi[probe_Pt>12&&probe_Pt<23&&probe_L1Pt_Bx0>15&&probe_L1Pt_Bx0<=26&&probe_L1Bx==0]')
            df_eg[i] = df_eg[i].Define('probeL1EG15to26Bxplus1_Eta',
                    'probe_Eta[probe_Pt>12&&probe_Pt<23&&probe_L1Pt_Bxplus1>15&&probe_L1Pt_Bxplus1<=26&&probe_L1Bx==1]')
            df_eg[i] = df_eg[i].Define('probeL1EG15to26Bxplus1_Phi',
                    'probe_Phi[probe_Pt>12&&probe_Pt<23&&probe_L1Pt_Bxplus1>15&&probe_L1Pt_Bxplus1<=26&&probe_L1Bx==1]')
...
def ZMuMu_Plots(df, nbbins, runmin, runmax):
    ...
    for i in range(3):
        if i ==0:
            ...
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bx0','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, 0)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bxmin1','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, -1)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bxplus1','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, 1)')
        if i ==1:
            df_mu[i] = df.Define('probe_idxL1jet','FindL1ObjIdx(_L1mu_eta, _L1mu_phi, probe_Eta, probe_Phi, _L1mu_Qual, 8)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bx0','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, 0, _L1mu_Qual, 8)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bxmin1','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, -1, _L1mu_Qual, 8)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bxplus1','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, 1, _L1mu_Qual, 8)')
        if i ==2:
            df_mu[i] = df.Define('probe_idxL1jet','FindL1ObjIdx(_L1mu_eta, _L1mu_phi, probe_Eta, probe_Phi, _L1mu_Qual, 12)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bx0','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, 0, _L1mu_Qual, 12)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bxmin1','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, -1, _L1mu_Qual, 12)')
            df_mu[i] = df_mu[i].Define('probe_idxL1jet_Bxplus1','FindL1ObjIdx_setBx(_L1mu_eta, _L1mu_phi, _L1mu_bx, probe_Eta, probe_Phi, 1, _L1mu_Qual, 12)')
        ...
        df_mu[i] = df_mu[i].Define('probe_L1Pt_Bx0',
                'GetVal(probe_idxL1jet_Bx0, _L1mu_pt)')
        df_mu[i] = df_mu[i].Define('probe_L1Qual_Bx0',
                'GetVal(probe_idxL1jet_Bx0, _L1mu_Qual)')

        df_mu[i] = df_mu[i].Define('probe_L1Pt_Bxmin1',
                'GetVal(probe_idxL1jet_Bxmin1, _L1mu_pt)')
        df_mu[i] = df_mu[i].Define('probe_L1Qual_Bxmin1',
                'GetVal(probe_idxL1jet_Bxmin1, _L1mu_Qual)')

        df_mu[i] = df_mu[i].Define('probe_L1Pt_Bxplus1',
                'GetVal(probe_idxL1jet_Bxplus1, _L1mu_pt)')
        df_mu[i] = df_mu[i].Define('probe_L1Qual_Bxplus1',
                'GetVal(probe_idxL1jet_Bxplus1, _L1mu_Qual)')

        df_mu[i] = df_mu[i].Define('probe_L1PtoverRecoPt','probe_L1Pt/probe_Pt')
        
        df_mu[i] = makehistosforturnons_inprobeetaranges(df_mu[i], histos, etavarname='probe_Eta', phivarname='probe_Phi', ptvarname='probe_Pt', responsevarname='probe_L1PtoverRecoPt', etabins=muEtaBins, l1varname='probe_L1Pt', l1thresholds=[5,10,15,20,22,25],  prefix=label[i]+"_plots" , binning = leptonpt_bins, l1thresholdforeffvsrunnb = 22, offlinethresholdforeffvsrunnb = 27, nbbins = nbbins, runmin = runmin, runmax = runmax)
        ...
        if i== 2:
            ...
            df_mu[i] = df_mu[i].Define('probeL1Mu10to21Bx0_Eta',
                    'probe_Eta[probe_Pt>8&&probe_Pt<25&&probe_L1Pt>10&&probe_L1Pt<=21&&probe_L1Bx==0&&probe_L1Qual>=12]')
            df_mu[i] = df_mu[i].Define('probeL1Mu10to21Bx0_Phi',
                    'probe_Phi[probe_Pt>8&&probe_Pt<25&&probe_L1Pt>10&&probe_L1Pt<=21&&probe_L1Bx==0&&probe_L1Qual>=12]')
            ...
            histos['L1Mu22_bxmin1_etaphi'] = df_mu[i].Filter("Flag_IsUnprefirable")\
                    .Histo2D(
                        ROOT.RDF.TH2DModel('L1Mu22_bxmin1_etaphi', '',
                            100, -5,5, 100, -3.1416, 3.1416),
                        'probeL1Mu22Bxmin1_Eta',
                        'probeL1Mu22Bxmin1_Phi')

            histos['L1Mu22_bx0_etaphi'] = df_mu[i].Filter("Flag_IsUnprefirable")\
                    .Histo2D(
                        ROOT.RDF.TH2DModel('L1Mu22_bx0_etaphi', '',
                            100, -5,5, 100, -3.1416, 3.1416),
                        'probeL1Mu22Bx0_Eta',
                        'probeL1Mu22Bx0_Phi')
            ...
            histos['L1Mu22_bxmin1_eta'] = df_mu[i].Filter("Flag_IsUnprefirable")\
                    .Histo1D(
                        ROOT.RDF.TH1DModel('L1Mu22_bxmin1_eta', '',
                            100, -5, 5),
                        'probeL1Mu22Bxmin1_Eta')

            histos['L1Mu22_bx0_eta'] = df_mu[i].Filter("Flag_IsUnprefirable")\
                    .Histo1D(
                        ROOT.RDF.TH1DModel('L1Mu22_bx0_eta', '',
                            100, -5, 5),
                        'probeL1Mu22Bx0_Eta')

            histos['L1Mu22_bxplus1_eta'] = df_mu[i].Filter("Flag_IsUnprefirable")\
                    .Histo1D(
                        ROOT.RDF.TH1DModel('L1Mu22_bxplus1_eta', '',
                            100, -5, 5),
                        'probeL1Mu22Bxplus1_Eta')

            histos['L1Mu22_bxmin1_eta_noflag'] = df_mu[i].Histo1D(
                    ROOT.RDF.TH1DModel('L1Mu22_bxmin1_eta_noflag', '',
                        100, -5, 5),
                    'probeL1Mu22Bxmin1_Eta')

            histos['L1Mu22_bx0_eta_noflag'] = df_mu[i].Histo1D(
                    ROOT.RDF.TH1DModel('L1Mu22_bx0_eta_noflag', '',
                        100, -5, 5),
                    'probeL1Mu22Bx0_Eta')

            histos['L1Mu22_bxplus1_eta_noflag'] = df_mu[i].Histo1D(
                    ROOT.RDF.TH1DModel('L1Mu22_bxplus1_eta_noflag', '',
                        100, -5, 5),
                    'probeL1Mu22Bxplus1_Eta')
    return df, histos
...
def AnalyzeCleanJets(df, JetRecoPtCut, L1JetPtCut, nbbins, runmin, runmax):    
    ...
    for i in range(len(jetEtaBins)-1):
        ...
        histos[str_bineta+'_ResponseVsRunNb'] = df_JetsBinnedInEta.Histo2D(
        ROOT.RDF.TH2DModel('h_JetResponseVsRunNb_{}'.format(str_bineta), 
        '', nbbins, runmin, runmax, 100, 0 , 2), 
        'runnb', 'cleanJetL1OverRecoPt_EtaRestricted')
    ...
    histos['L1Jet100to150_bxmin1_eta'] = df.Histo1D(ROOT.RDF.TH1DModel('L1Jet100to150_bxmin1_eta', '', 100, -5, 5), 'probeL1Jet100to150Bxmin1_Eta')
...
def HFNoiseStudy(df, nbbins, runmin, runmax):
    histos={}
    ...
    for i, s in enumerate(suffix):
        ...
        histos[s+'_runnb'] = df_passvsfailL1[i].Histo1D(
                ROOT.RDF.TH1DModel(
                    s+'_runnb', '', nbbins, runmin, runmax)
                , '_runNb')
...
def AnalyzePtBalance(df, nbbins, runmin, runmax):
    histos = {}
    df_JetsBinnedInEta ={}
    ...
    for i in range(len(jetEtaBins)-1):
        ...
        histos['RecoJetvsRunNb'+str_bineta] = df_JetsBinnedInEta[str_bineta].Histo2D(
                ROOT.RDF.TH2DModel(
                    'h_PtBalanceVsRunNb_{}'.format(str_bineta),
                    'ptbalance', nbbins, runmin, runmax, 100,0,2),
                '_runNb','ptbalance')
        histos['L1JetvsRunNb'+str_bineta] = df_JetsBinnedInEta[str_bineta].Histo2D(
                ROOT.RDF.TH2DModel(
                    'h_L1PtBalanceVsRunNb_{}'.format(str_bineta), 
                    'ptbalanceL1', nbbins, runmin, runmax, 100,0,2),
                '_runNb','ptbalanceL1')
...
def RunNbLimits(df):
    ...
    #quit()
    return(nb_bins, runNb_min, runNb_max)
